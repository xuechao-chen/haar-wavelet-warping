...
class WarpingAppOnGPU1 : public GLApp
{
...
public:
	WarpingAppOnGPU1(unsigned int width, unsigned int height, const std::string& name) : GLApp(width, height, name){}

protected:
	void onInit() override
	{
		m_pQuadProgram = ShaderProgram::create("quad_tex2d_array.vert", "quad_tex2d_array.frag");
		m_pWarpingProgram = ShaderProgram::create("warping_on_gpu_1.comp");

		std::vector<const char*> PathArray;
		for (size_t i = 0; i < 256; i++)
		{
			PathArray.push_back("tex.jpg");
		}

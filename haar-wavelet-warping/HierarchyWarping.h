...
{
public:
	static void warp(const Haar2D& haar2D, std::vector<Point2D>& samples)
	{
		...
		for (uint16_t i = 0; i < SliceNum - 1; i++)
		{
			...
			for (uint16_t u = 0; u < W; u++)
			{
				for (uint16_t v = 0; v < H; v++)
				{
					...
					warpX({ {u * InvW, v * InvH}, {InvW, InvH * RatioY} }, RatioX1, samples);
					warpX({ {u * InvW, v * InvH + InvH * RatioY}, {InvW, InvH * (1.0f - RatioY)} }, RatioX2, samples);
				}
			}
		}
	}
...
private:
	static void warpX(const Region2D& range2d, double ratio, std::vector<Point2D>& samples)
	{
		for (auto& sample : samples)
		{
			...
			{
				sample.x = x2 - (x2 - sample.x) * (0.5f / (1 - ratio));
			}
		}
	}
	...
	static void warpY(const Region2D& range2d, double ratio, std::vector<Point2D>& samples)
	{
		for (auto& sample : samples)
		{
			...
			{
				sample.y = y2 - (y2 - sample.y) * (0.5f / (1 - ratio));
			}
		}
	}
};
